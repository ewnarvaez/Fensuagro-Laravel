...
class Menu extends Model
{
    ...
    public function getPadres($front)
    {
        if ($front) {
            return $this->whereHas('roles', function ($query) {
                $query->where('rolid', session()->get('rolid'))->orderby('menuid');
            })->where('estado', 1)
                ->orderby('menuid')
                ->orderby('orden')
                ->get()
    ...
    public function guardarOrden($menu)
    {
        ...
        foreach ($menus as $var => $value) {
            $this->where('id', $value->id)->update(['menuid' => 0, 'orden' => $var + 1]);
            if (!empty($value->children)) {
                foreach ($value->children as $key => $vchild) {
                    $update_id = $vchild->id;
                    $parent_id = $value->id;
                    $this->where('id', $update_id)->update(['menuid' => $parent_id, 'orden' => $key + 1]);
